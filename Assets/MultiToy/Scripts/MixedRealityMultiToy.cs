...
public class MixedRealityMultiToy : MonoBehaviour
{
    static public MixedRealityMultiToy Instance = null;
    ...
    private void Start()
    {
        MixedRealityManager.Instance.OnHandClosedDelegate += HandClosed;
    }
    ...
    void HandClosed()
    {
        if (MixedRealityManager.Instance._currentChapter >= MixedRealityManager.GameChapter.SearchForOppy)
        {
    ...
    void Update()
    {
        MixedRealityManager.GameChapter currentChapter = MixedRealityManager.Instance._currentChapter;

        Vector3 controllerPos = Vector3.zero;
        Quaternion controllerRot = Quaternion.identity;
        MixedRealityManager.Instance.GetDominantHand(ref controllerPos, ref controllerRot);

        if (currentChapter != MixedRealityManager.GameChapter.OppyBaitsYou)
        {
        ...
        if (!TriggerHeld() && _canSwitchToys)
        {
            ...
            if (lastToy != _toyIndexController)
            {
                ...
                _switchToy_1.Play();
                MixedRealityTutorial.Instance.HideMessage(MixedRealityTutorial.TutorialMessage.SwitchToy);

                SetToyMesh((ToyOption)_toyIndexController);
            }
        }

        if (currentChapter == MixedRealityManager.GameChapter.SearchForOppy)
        {
            Vector3 oppyPos = MixedRealityManager.Instance._pet.transform.position + Vector3.up * 0.2f;
            float oppyDot = Vector3.Dot(GetFlashlightDirection(), (oppyPos - transform.position).normalized);
            bool oppyActive = MixedRealityManager.Instance._pet.gameObject.activeSelf;
            if (!MixedRealityManager.Instance._oppyDiscovered)
            {
                if (oppyDot >= 0.95f && oppyActive)
                {
                    MixedRealityManager.Instance.PlayOppyDiscoveryAnim();
                    MixedRealityManager.Instance._pet.PlaySparkles(false);
                }
            }
        }
        ...
        if (MixedRealityManager.Instance._usingHands)
        {
            _ballTossCooldownFactor += Time.deltaTime;
            currentToyIndex = _toyIndexHand;
            if (currentChapter >= MixedRealityManager.GameChapter.SearchForOppy)
            {
                Transform mainCam = MixedRealityManager.Instance._mainCamera.transform;
                ...
                float fistStrength = Mathf.Clamp01((MixedRealityManager.Instance._fistValue - 0.3f) * 5.0f);
                ...
                if (lastToy != currentToyIndex)
                {
                    ...
                    if (currentToyIndex > (int)ToyOption.None)
                    {
                        ...
                        {
                            MixedRealityTutorial.Instance.HideMessage(MixedRealityTutorial.TutorialMessage.EnableFlashlight);
                        }
        ...
        if (currentChapter >= MixedRealityManager.GameChapter.SearchForOppy)
        {
            if (currentToyIndex > (int)ToyOption.None)
            {
                if (TriggerPressed())
                {
                    if (currentToyIndex == (int)ToyOption.BallGun && MixedRealityManager.Instance._ballCount > 0)
                    {
            ...
            BallCollectable tbc = MixedRealityManager.Instance.GetTargetedBall(transform.position, transform.forward);
            if (tbc)
            {
                    ...
                    tbc._ballState == BallCollectable.BallStatus.Released)
                {
                    if (!MixedRealityManager.Instance._usingHands)
                    {
                        if (IsFlashlightAbsorbing())
                        {
                            ...
                            {
                                tbc.AbsorbBall();
                                MixedRealityManager.Instance.DiscoveredBall(_grabbedBall);
                            }
                        }
                    }
                }
    ...
    /// </summary>
    public void SetToy(MixedRealityManager.GameChapter forcedChapter)
    {
        switch (forcedChapter)
        {
            case MixedRealityManager.GameChapter.Title:
            case MixedRealityManager.GameChapter.Introduction:
                DeactivateAllToys();
                ShowToy(false);
                _toyIndexController = (int)ToyOption.Flashlight;
                ...
                break;
            case MixedRealityManager.GameChapter.OppyBaitsYou:
                DeactivateAllToys();
                ShowToy(false);
                _canSwitchToys = false;
                _flashLightUnlocked = false;
                _toyFlashlight.SetLightStrength(MixedRealityManager.Instance._usingHands ? 0.0f : 1.0f);
                break;
            case MixedRealityManager.GameChapter.SearchForOppy:
                ShowToy(true);
                _canSwitchToys = false;
                break;
            case MixedRealityManager.GameChapter.OppyExploresReality:
                DeactivateAllToys();
                ShowToy(true);
                ...
                _wallToyUnlocked = false;
                break;
            case MixedRealityManager.GameChapter.TheGreatBeyond:
                ShowToy(true);
                _canSwitchToys = true;
                if (!MixedRealityManager.Instance._usingHands)
                {
                    MixedRealityTutorial.Instance.DisplayMessage(MixedRealityTutorial.TutorialMessage.SwitchToy);
                }
                break;
            case MixedRealityManager.GameChapter.Ending:
                _canSwitchToys = true;
                break;
        }
    }
    ...
    public void UnlockBallShooter()
    {
        if (!MixedRealityManager.Instance._usingHands)
        {
            ...
            if (MixedRealityManager.Instance._ballCount == 0)
            {
                // Give two free balls so that you don't get stuck
                MixedRealityManager.Instance._ballCount = 2;
            }
            _unlockToy_1.Play();
            MixedRealityTutorial.Instance.DisplayMessage(MixedRealityTutorial.TutorialMessage.ShootBall);
            _toys[(int)ToyOption.Flashlight].Deactivate();
    ...
    public void UnlockWallToy()
    {
        ...
        if (!MixedRealityManager.Instance._usingHands)
        {
            MixedRealityTutorial.Instance.DisplayMessage(MixedRealityTutorial.TutorialMessage.ShootWall);
        ...
        {
            MixedRealityTutorial.Instance.DisplayMessage(MixedRealityTutorial.TutorialMessage.AimWall);
        }
    ...
    bool TriggerPressed()
    {
        OVRInput.RawButton triggerButton = MixedRealityManager.Instance._gameController == OVRInput.Controller.RTouch ? OVRInput.RawButton.RIndexTrigger : OVRInput.RawButton.LIndexTrigger;
    ...
    {
        bool usingRightController = MixedRealityManager.Instance._gameController == OVRInput.Controller.RTouch;
    ...
    public Vector3 GetMuzzlePosition()
    {
        ...
        if (MixedRealityManager.Instance._usingHands)
        {
            OVRSkeleton refHand = MixedRealityManager.Instance.GetActiveHand();
    ...
    {
        if (MixedRealityTutorial.Instance._currentMessage == MixedRealityTutorial.TutorialMessage.AimWall)
        {
    ...
    public void GrabBall(BallCollectable bc)
    {
        ...
        _grabbedBall = bc;
        MixedRealityManager.Instance.DiscoveredBall(bc);
        MixedRealityTutorial.Instance.HideMessage(MixedRealityTutorial.TutorialMessage.BallSearch);
    ...
    public void UseHands(bool usingHands, bool rightHand = false)
    {
        if (usingHands)
        {
            if (_toyVisible && MixedRealityManager.Instance._currentChapter >= MixedRealityManager.GameChapter.SearchForOppy)
            {
