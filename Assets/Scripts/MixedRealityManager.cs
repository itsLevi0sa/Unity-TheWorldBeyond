...
{
    static public MixedRealityManager Instance = null;
    ...
    [Header("Game Pieces")]
    public MixedRealityVirtualPet _pet;
    ...
    public void Update()
    {
        ...
        if (usingHands != _usingHands)
        {
            ...
            MixedRealityTutorial.Instance.UpdateMessageTextForInput();
        }
        ...
        if (_currentChapter >= GameChapter.Title)
        {
            ...
            {
                MixedRealityTutorial.Instance.DisplayMessage(MixedRealityTutorial.TutorialMessage.ERROR_USER_WALKED_OUTSIDE_OF_ROOM);
            }
            else if (MixedRealityTutorial.Instance._currentMessage == MixedRealityTutorial.TutorialMessage.ERROR_USER_WALKED_OUTSIDE_OF_ROOM)
            {
                MixedRealityTutorial.Instance.DisplayMessage(MixedRealityTutorial.TutorialMessage.None);
            }
        }
    ...
    private void UpdateHandVisibility(bool handHidden, DistantInteractionLineVisual interactionLine, float previousHandToBallDistance, bool primary, [CanBeNull] BallCollectable grabbedBall)
    {
        ...
        if (handHidden)
        {
            ...
            if (primary) MixedRealityTutorial.Instance.ForceInvisible();
        ...
        else
        {
            ...
            if (primary) MixedRealityTutorial.Instance.ForceVisible();
    ...
    public void ForceChapter(GameChapter forcedChapter)
    {
        ...
        MixedRealityMultiToy.Instance.SetToy(forcedChapter);
        _currentChapter = forcedChapter;
        MixedRealityEnvironment.Instance.ShowEnvironment((int)_currentChapter > (int)GameChapter.SearchForOppy);
        ...
        switch (_currentChapter)
        {
            case GameChapter.Title:
                ...
                MixedRealityTutorial.Instance.DisplayMessage(MixedRealityTutorial.TutorialMessage.None);
                MixedRealityEnvironment.Instance._sun.enabled = false;
                break;
            case GameChapter.Introduction:
            ...
            case GameChapter.SearchForOppy:
                ...
                _passthroughStylist.ResetPassthrough(0.1f);
                MixedRealityEnvironment.Instance._sun.enabled = true;
    ...
    IEnumerator FlickerCameraToClearColor()
    {
        ...
        while (timer <= flickerTimer)
        {
            ...
            if (timer >= flickerTimer)
            {
                ...
                MixedRealityEnvironment.Instance.ShowEnvironment(true);
            }
            yield return null;
        }
    }
    ...
    IEnumerator CountdownToFlashlight(float spawnTime)
    {
        ...
        if (_usingHands)
        {
            MixedRealityTutorial.Instance.DisplayMessage(MixedRealityTutorial.TutorialMessage.EnableFlashlight);
        }
    ...
    void GetRoomFromScene()
    {
        ...
        try
        {
            ...
            if (!_vrRoom.IsPlayerInRoom())
            {
                MixedRealityTutorial.Instance.DisplayMessage(MixedRealityTutorial.TutorialMessage.ERROR_USER_STARTED_OUTSIDE_OF_ROOM);
            }
            MixedRealityEnvironment.Instance.Initialize();
            ForceChapter(GameChapter.Title);
        }
        catch
        {
            // if initialization messes up for some reason, quit the app
            MixedRealityTutorial.Instance.DisplayMessage(MixedRealityTutorial.TutorialMessage.ERROR_NO_SCENE_DATA);
        }
    }
    ...
    public void DiscoveredBall(BallCollectable collected)
    {
        ...
        MixedRealityTutorial.Instance.HideMessage(MixedRealityTutorial.TutorialMessage.NoBalls);
    ...
    IEnumerator MalfunctionFlashlight()
    {
        ...
        MixedRealityEnvironment.Instance.FlickerSun();
    ...
    IEnumerator FlickerFlashlight(float delayTime = 0.0f)
    {
        ...
        if (_oppyDiscoveryCount == 1)
        {
            MixedRealityTutorial.Instance.DisplayMessage(MixedRealityTutorial.TutorialMessage.BallSearch);
    ...
    public void OpenedWall(int wallID)
    {
        ...
        if (_currentChapter == MixedRealityManager.GameChapter.OppyExploresReality)
        {
            MixedRealityTutorial.Instance.DisplayMessage(MixedRealityTutorial.TutorialMessage.None);
    ...
    IEnumerator DoEndingSequence()
    {
        ...
        if (_spaceShipAnimator)
        {
            ...
            while (timer < flyingAwayTime)
            {
                ...
                MixedRealityEnvironment.Instance.FadeOutdoorAudio(1 - fadeValue);
                if (timer >= flyingAwayTime)
                {
                    MixedRealityEnvironment.Instance.SetOutdoorAudioParams(Vector3.zero, false);
    ...
    {
        _floorHeight = height;
        MixedRealityEnvironment.Instance.MoveGroundFloor(height);
    }
}
