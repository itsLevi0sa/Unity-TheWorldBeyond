...
public class MixedRealityVirtualPet : MonoBehaviour
{
    ...
    public void LateUpdate()
    {
        ...
        if (!_mainCam)
        {
            _mainCam = MixedRealityManager.Instance._mainCamera.transform;
        }
        ...
        switch (_oppyState)
        {
            ...
            case PetState.Ending:
                ...
                Vector3 targetPos = _rampBaseHit ? MixedRealityManager.Instance._finalUfoTarget.position + Vector3.up * 3.0f : MixedRealityManager.Instance._finalUfoRamp.position;
                ...
                if (targetDistance.magnitude <= dist)
                {
                    ...
                    {
                        _rampBaseHit = true;
                        _agent.SetDestination(MixedRealityManager.Instance._finalUfoTarget.position + Vector3.up * 3.0f);
                ...
                if (endGame)
                {
                    MixedRealityManager.Instance.FlyAwayUFO();
    ...
    void DoLookAtBehavior(bool instantLook, bool eyesAlso)
    {
        ...
        if (MixedRealityManager.Instance._currentChapter == MixedRealityManager.GameChapter.OppyExploresReality
    ...
    void DoChaseBehavior()
    {
        ...
        // get the closest one
        BallCollectable ballBC = MixedRealityManager.Instance.GetClosestEdibleBall(transform.position);
        if (MixedRealityManager.Instance._currentChapter == MixedRealityManager.GameChapter.OppyExploresReality
            && MultiToy.Instance.IsWalltoyActive())
        {
        ...
        if (ballBC)
        {
            ...
            if (ballDirectionToOppy.magnitude > 0.5f)
            {
                ...
                {
                    SetMoveTarget(new Vector3(ballBC.gameObject.transform.position.x, MixedRealityManager.Instance.GetFloorHeight(), ballBC.gameObject.transform.position.z), -ballDirectionToOppy.normalized, true);
                }
            }
    ...
    void CheckForPetting()
    {
        ...
        {
            Transform Lhand = MixedRealityManager.Instance._leftHandAnchor;
            Transform Rhand = MixedRealityManager.Instance._rightHandAnchor;
            if (MixedRealityManager.Instance._usingHands)
            {
                if (MixedRealityManager.Instance._leftHand && MixedRealityManager.Instance._rightHand)
                {
                    Lhand = MixedRealityManager.Instance._leftHand.Bones[9].Transform;
                    Rhand = MixedRealityManager.Instance._rightHand.Bones[9].Transform;
                }
            }
    ...
    public void SetOppyChapter(MixedRealityManager.GameChapter newChapter)
    {
        StopAllCoroutines();
        ResetAnimFlags();
        switch (newChapter)
        {
            case MixedRealityManager.GameChapter.Title:
                MixedRealityManager.Instance._spaceShipAnimator.StopIdleSound();
                break;
            ...
            case MixedRealityManager.GameChapter.Ending:
                EndLookTarget();
                _agent.SetDestination(MixedRealityManager.Instance._finalUfoRamp.position);
                _rampBaseHit = false;
    ...
    public void FinishEating()
    {
        ...
        if (_collectedBalls >= MixedRealityManager.Instance._oppyTargetBallCount)
        {
    ...
    public void GoToUFO()
    {
        MixedRealityManager.Instance.ForceChapter(MixedRealityManager.GameChapter.Ending);
    }
    #endregion AnimEvents
    public bool IsGameOver()
    {
        bool gameOver = MixedRealityManager.Instance._currentChapter == MixedRealityManager.GameChapter.Ending;
    ...
    {
        bool canListen = _collectedBalls < MixedRealityManager.Instance._oppyTargetBallCount;
        canListen &= MixedRealityManager.Instance._currentChapter == MixedRealityManager.GameChapter.TheGreatBeyond;
    ...
    {
        Vector3 currTargetPos = new Vector3(worldPosition.x, MixedRealityManager.Instance.GetFloorHeight(), worldPosition.z);

        if (Vector3.Distance(_moveTargetPos, currTargetPos) > 1.0f || force)
        {
            _moveTargetPos = currTargetPos;
            _moveTargetDir = new Vector3(faceDirection.x, MixedRealityManager.Instance.GetFloorHeight(), faceDirection.z).normalized;
    ...
    void ApproachPlayer()
    {
        ...
        _agent.speed = _runSpeed;
        _agent.SetDestination(new Vector3(targetPos.x, MixedRealityManager.Instance.GetFloorHeight(), targetPos.z));
    }
    ...
    public void MoveAway()
    {
        ...
        Vector3 targetPoint = _mainCam.position + new Vector3(point.x, MixedRealityManager.Instance.GetFloorHeight(), point.y);
        targetPoint.y = 0;
