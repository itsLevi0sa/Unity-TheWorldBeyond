...
public class MixedRealitySceneLoader : OVRSceneModelLoader
{
    ...
    IEnumerator AwaitSceneModel()
    {
        ...
        var preamble = $"[{nameof(MixedRealitySceneLoader)}]: {nameof(SceneManager.LoadSceneModel)}";
        var attemptsRemaining = attemptCount;
        while (attemptsRemaining > 0)
        {
            if (SceneManager.LoadSceneModel())
            {
                Debug.Log($"[{nameof(MixedRealitySceneLoader)}]: Loading scene model...");
                yield break;
            }
    ...
    IEnumerator AwaitSceneCaptureCompletion()
    {
        ...
        Debug.Log($"[{nameof(MixedRealitySceneLoader)}]: Requesting Scene Capture.");
        if (SceneManager.RequestSceneCapture())
        {
            ...
            Debug.LogError($"[{nameof(MixedRealitySceneLoader)}]: Did not get a response following the completion of Scene Capture.");
        }
        else
        {
            ...
            {
                MixedRealityTutorial.Instance.DisplayMessage(MixedRealityTutorial.TutorialMessage.ERROR_NO_SCENE_DATA_LINK);
            }

            Debug.LogError($"[{nameof(MixedRealitySceneLoader)}]: {nameof(SceneManager.RequestSceneCapture)} returned false.");
        }
    ...
    protected override void OnSceneCaptureReturnedWithoutError()
    {
        ...
        Debug.Log($"[{nameof(MixedRealitySceneLoader)}]: Scene Capture completed successfully.");
    ...
    {
        sceneCaptureComplete = true;
        Debug.LogError($"[{nameof(MixedRealitySceneLoader)}]: Scene Capture failed with an unexpected error.");
